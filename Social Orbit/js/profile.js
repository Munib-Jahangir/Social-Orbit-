...
import { getUserStats } from './user.js';
...
function setupEventListeners() {
    document.getElementById('postsTab').addEventListener('click', () => switchTab('posts'));
...
function switchTab(tab) {
    ...
    document.getElementById(tab === 'posts' ? 'postsTab' : 'savedTab').classList.add('active');
...
function renderPosts() {
    ...
    const postsList = currentTab === 'posts' ? posts.getByUser(currentUser.id) : posts.getSaved();
    
    if (postsList.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <div class="empty-icon">${currentTab === 'posts' ? '📝' : '⭐'}</div>
    ...
    container.innerHTML = postsList.map(post => renderPost(post)).join('');
}
